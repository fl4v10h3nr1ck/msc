...
final class Util {
	...
	public function ultimoPagamento(&$bd, $id_solicitacao){
		...
		SELECT pag.id_pagamento, pag.num_licencas, pag.status, pag.num_de_dias 
		FROM pagamentos as pag inner join solucoes as sol on pag.fk_solucao = sol.id_solucao 
		WHERE pag.fk_solicitacao = ".$id_solicitacao." AND sol.codigo = '".$_GET["softCOD"]."' AND pag.status_pagamento='CONFIRMADO' AND 
		pag.mes<=".intval(date("m"))." AND pag.ano<=".intval(date("Y"))." AND pag.tipo='LICENCA' 
		AND pag.a_partir_de <= '".date("Y-m-d")."' ORDER BY pag.id_pagamento DESC");
	...
	public function fechaPagamento(&$bd, $id_pagamento, $num_licencas){
		...
		if(count($reg_ativados) >= $num_licencas)	
			return $bd->atualiza("pagamentos", "id_pagamento", $id_pagamento, array("status"), array("FECHADO"));
