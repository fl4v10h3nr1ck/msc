...
if($util->permitir()===false)
	...
	$pagamento  = $util->ultimoPagamento($bd, $reg_solicitacao['id_solicitacao']);
	...
	if($util->addChave($bd, $_GET["ID"], $pagamento['id_pagamento'], $chave, $bd)===false){
	...
	if($util->fechaPagamento($bd, $pagamento['id_pagamento'], $pagamento['num_licencas'])===false){
